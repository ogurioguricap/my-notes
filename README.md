...
> 这一层的底线由 **573 项断言**守着（`tools/test-notebook.mjs`）：数据层（页面管理 / 垃圾桶 / 闪卡间隔 / 录音锚点 / 备份导入导出 / 老数据迁移）、
...
│   ├── test-notebook.mjs       GoodNotes 模式实测（笔记本 / 页面 / 四种笔 / 闪卡·复习中心（筛选 / 热图 / Anki）/ 录音转写·分片·分段锚定 / 朗读 / PDF 文字层·目录页层级·内链·阅读版·省纸排版（2 页拼一张 / 骑马钉）/ 长图 / 批量导出 / EPUB（层级目录·封面·内链·NCX）·单文件 HTML / OCR 定位·选区·原位替换·跨本批量队列·识别质量清单 / 封面图 / 命中高亮 / 模板库 / Markdown 导出 / 手势，573 项断言）
...
| 11 | GoodNotes 模式实测 573 项断言（笔记本 / 页面 / 四种笔 / 闪卡·复习中心（筛选复习 / 热图 / Anki CSV）/ 录音转写·分片·分段锚定 / 页面朗读 / PDF 画质·文字层·目录页（含层级）·内链·阅读版·省纸排版（2 页拼一张 / 骑马钉页序）/ 长图 / 批量导出 / EPUB（层级目录 / 封面 / 内链 / NCX / 无死链校验）·单文件 HTML / OCR 定位·选区·原位替换·跨本批量队列（续跑 / 重试 / 不重复花钱） / 封面图 / 命中高亮 / 模板库 / Markdown 导出 / 手势） | **中止** |
