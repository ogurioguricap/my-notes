...
> 这一层的底线由 **694 项断言**守着（`tools/test-notebook.mjs`）：数据层（页面管理 / 垃圾桶 / 闪卡间隔 / 录音锚点 / 备份导入导出 / 老数据迁移）、
...
│   ├── test-notebook.mjs       GoodNotes 模式实测（笔记本 / 页面 / 四种笔 / 闪卡·复习中心（筛选 / 热图 / Anki）/ 录音转写·分片·分段锚定 / 朗读 / PDF 文字层·目录页层级·内链·阅读版·省纸排版（2 页拼一张 / 骑马钉）/ 长图 / 批量导出 / EPUB（层级目录·封面·内链·NCX）·单文件 HTML / OCR 定位·选区·原位替换·跨本批量队列·识别质量清单 / 复习卡来源页 / 跨页拖拽 / 书签页内落点 / 打印检查单 / 卡片来源整理 / 页面总览批量管理 / 封面图 / 命中高亮 / 模板库 / Markdown 导出 / 手势，694 项断言）
...
| 8 | 运行时冒烟测试（真跑一遍：资料库 → 打开笔记 → 标注层 → 编辑器 → **笔记本模式**（含识别队列、复习中心、质量清单、跨页拖拽、打印检查单、卡片来源整理、页面总览批量管理）→ 搜索，90 项断言） | **中止** |
| 9 | 标注编辑实测 101 项断言（撤销 / 套索 / 橡皮 / 形状 / Canvas 契约） | **中止** |
| 10 | PWA 自检（manifest / 图标 / Service Worker 49 项） | **中止** |
| 11 | GoodNotes 模式实测 694 项断言（笔记本 / 页面 / 四种笔 / 闪卡·复习中心（筛选复习 / 热图 / Anki CSV）/ 录音转写·分片·分段锚定 / 页面朗读 / PDF 画质·文字层·目录页（含层级）·内链·阅读版·省纸排版（2 页拼一张 / 骑马钉页序）/ 长图 / 批量导出 / EPUB（层级目录 / 封面 / 内链 / NCX / 无死链校验）·单文件 HTML / OCR 定位·选区·原位替换·跨本批量队列（续跑 / 重试 / 不重复花钱） / 封面图 / 命中高亮 / 模板库 / Markdown 导出 / 手势） | **中止** |
