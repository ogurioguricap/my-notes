...
| **导出 PDF（三档画质 + 可搜索文字层）** | 笔记本「导出 ▾」：标准 96dpi / 高清 192dpi（默认）/ 打印级 288dpi（约 9 倍体积，正式打印用）；导出前会**等页面里的图片加载完**（不会把占位框画进 PDF）；**导出的 PDF 带隐形文字层**——文本框内容按原位、已 OCR 的手写按行铺进去，阅读器里 **Ctrl+F 能搜、能选中复制**（可在菜单里关掉）；勾「导出时撕掉胶带」可导出成「看答案用」的版本；另有 2× PNG 与打印 |
| **批量导出** | 资料库进多选 → 「导出 PDF」→ 选画质 + 「合并成一个 PDF / 每本一个 PDF」→ 逐本下载时会有进度提示（导出的文件同样带可搜索文字层） |
...
│   ├── test-notebook.mjs       GoodNotes 模式实测（笔记本 / 页面 / 四种笔 / 闪卡 / 录音 / PDF 文字层与画质 / 批量导出 / OCR 定位 / 模板库 / Markdown 导出 / 同步，259 项断言）
...
| 11 | GoodNotes 模式实测 259 项断言（笔记本 / 页面 / 四种笔 / 闪卡 / 录音 / PDF 画质·文字层·合成 / 批量导出 / OCR 定位 / 模板库 / Markdown 导出 / 同步 / 裁剪） | **中止** |
