...
| **导出 EPUB / 单文件 HTML** | 导出菜单新增两种「发出去就能看」的格式：**EPUB**（每页一张图 + 这一页的文字/手写识别，任意阅读器可打开、可搜索，目录按页生成；含手写识别的内容照样能搜）与**单文件 HTML**（图片内联成 data URL，一个文件双击就开、带打印分页样式）。两者都是**浏览器里现写 ZIP / HTML 字节**，不依赖任何库 |
...
> 这一层的底线由 **489 项断言**守着（`tools/test-notebook.mjs`）：数据层（页面管理 / 垃圾桶 / 闪卡间隔 / 录音锚点 / 备份导入导出 / 老数据迁移）、
...
│   ├── test-notebook.mjs       GoodNotes 模式实测（笔记本 / 页面 / 四种笔 / 闪卡·复习中心（筛选 / 热图 / Anki）/ 录音转写·分片·分段锚定 / 朗读 / PDF 文字层·目录页层级·内链·阅读版 / 长图 / 批量导出 / EPUB·单文件 HTML / OCR 定位·选区·原位替换·跨本批量队列 / 封面图 / 命中高亮 / 模板库 / Markdown 导出 / 手势，489 项断言）
...
| 11 | GoodNotes 模式实测 489 项断言（笔记本 / 页面 / 四种笔 / 闪卡·复习中心（筛选复习 / 热图 / Anki CSV）/ 录音转写·分片·分段锚定 / 页面朗读 / PDF 画质·文字层·目录页（含层级）·内链·阅读版 / 长图 / 批量导出 / EPUB·单文件 HTML / OCR 定位·选区·原位替换·跨本批量队列（续跑 / 重试 / 不重复花钱） / 封面图 / 命中高亮 / 模板库 / Markdown 导出 / 手势） | **中止** |
