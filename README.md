...
| **录音转写（音频也能搜）** | 录音面板里每段录音都有「转文字」：走 SiliconFlow 的 ASR（与 OCR 同一家的 Key），转写结果挂在录音上 → **笔记本内搜索、资料库搜索、首页搜索**都能搜到录音里说过的内容；转写可单独清掉 |
| **笔记本内搜索** | 笔记本顶部 🔎：在这一本里搜文字、手写识别结果与录音转写，结果按页列出、标出来源（文字 / 手写识别 / 录音转写）、命中关键词高亮，点一下跳到那一页 |
...
│   ├── test-notebook.mjs       GoodNotes 模式实测（笔记本 / 页面 / 四种笔 / 闪卡 / 录音转写 / PDF 文字层·目录·内链 / 长图 / 批量导出 / OCR 定位与选区 / 模板库 / Markdown 导出 / 手势，318 项断言）
...
| 11 | GoodNotes 模式实测 318 项断言（笔记本 / 页面 / 四种笔 / 闪卡 / 录音与转写 / PDF 画质·文字层·目录·内链 / 长图 / 批量导出 / OCR 定位与选区 / 模板库 / Markdown 导出 / 手势） | **中止** |
